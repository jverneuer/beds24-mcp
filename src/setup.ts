...
import { packageRoot } from "./sdk/paths.ts";
...
export function serverAbsPath(): string {
	// <repo>/src/server.ts — valid when running from source via `bun run`.
	return join(packageRoot(), "src", "server.ts");
}
...
export async function runSetup(opts: SetupOptions = {}): Promise<void> {
	...
	// Install deps + build the index.
	const repoRoot = packageRoot();
	let ok = true;
	if (!opts.skipInstall) {
		ok = runStep("bun install", ["bun", "install"], repoRoot) && ok;
	}
	if (!opts.skipIndex) {
		ok = runStep("bun run index  (builds vector index from knowledge/)", ["bun", "run", "index"], repoRoot) && ok;
	}
