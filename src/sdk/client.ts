...
/** Resolve the facts dir for client-side request validation. */
function resolveFactsDir(): string {
	return process.env.BEDS24_FACTS_DIR ?? defaultKnowledgeDir();
}
...
export class Beds24Client {
	...
	async request<T = unknown>(
		...
		if (body !== undefined) {
			const schema = getSchema(resolveFactsDir(), endpoint, "request");
			if (schema && typeof schema === "object") {
				const check = validateRequest(resolveFactsDir(), endpoint, "request", body);
				if (!check.valid) {
					throw new Beds24Error({
