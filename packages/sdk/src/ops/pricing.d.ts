 ...
 */
export declare const CHANNEL_PRICE_MODEL: {
...
export declare class PricingOps {
    private client;
     ...
     */
    setDailyPrices(rows: CalendarWrite | CalendarWrite[]): Promise<Beds24Response<CalendarWriteResponse>>;
    /** Read the per-date calendar (daily prices + availability). */
    getCalendar(query: CalendarQuery): Promise<Beds24Response<CalendarReadResponse>>;
    /** Set fixed (date-range) prices. Max 100 fixed prices per room. */
    setFixedPrices(rows: FixedPriceWrite | FixedPriceWrite[]): Promise<Beds24Response<FixedPriceWriteResponse>>;
    /** Read fixed prices. */
    getFixedPrices(query?: FixedPriceQuery): Promise<Beds24Response<ResponseBodyOf<OpOf<"GET /inventory/fixedPrices">>>>;
    /**
     ...
     * channels not listed.
     */
    pushToChannel(channel: string, rows: CalendarWrite | CalendarWrite[]): Promise<{
        response: Beds24Response<CalendarWriteResponse>;
        model: PriceTransmissionModel;
    }>;
}
