...
// --- Properties (Beta reads/writes; DELETE Coming soon) --------------------------------
...
export class AccountOps {
	...
	async list(query: AccountQuery): Promise<Beds24Response<AccountListResponse>> {
	 ...
	 */
	async create(drafts: AccountDraft | AccountDraft[]): Promise<Beds24Response<AccountWriteResponse>> {
...
export class PropertyOps {
	...
	async list(query: PropertyQuery): Promise<Beds24Response<PropertyListResponse>> {
	 ...
	 */
	async create(drafts: PropertyDraft | PropertyDraft[]): Promise<Beds24Response<PropertyWriteResponse>> {
		const items = Array.isArray(drafts) ? drafts : [drafts];
		return this.client.request("POST /properties", items);
	}

	/** Delete properties by id (Coming soon; properties-and-rooms.md §7). */
	async remove(ids: number[]): Promise<Beds24Response<PropertyDeleteResponse>> {
	...
	async listRooms(query: RoomQuery): Promise<Beds24Response<RoomListResponse>> {
		return this.client.request("GET /properties/rooms", query);
	}

	/** Delete rooms by id (Coming soon; properties-and-rooms.md §7). */
	async removeRoom(ids: number[]): Promise<Beds24Response<RoomDeleteResponse>> {
...
export class OrganizationOps {
	...
	async listUsers(query?: OrganizationUserQuery): Promise<Beds24Response<OrganizationUserListResponse>> {
