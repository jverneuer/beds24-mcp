...
export type EndpointKey = {
    [P in keyof paths]: keyof paths[P] & string extends infer M ? M extends HttpMethod ? paths[P][M] extends never ? never : `${Uppercase<M>} ${P & string}` : never : never;
}[keyof paths];
/** The operation object (parameters + requestBody + responses) for an endpoint key. */
export type OpOf<E extends EndpointKey> = E extends `${infer M} ${infer P}` ? P extends keyof paths ? Lowercase<M> extends keyof paths[P] ? paths[P][Lowercase<M>] : never : never : never;
/**
 ...
 */
export type RequestBodyOf<Op> = [Op] extends [never] ? undefined : Op extends {
    requestBody?: {
        content: {
            "application/json": infer B;
        };
    };
} ? [B] extends [never] ? Op extends {
    parameters?: {
        query?: infer Q;
    };
} ? [Q] extends [never] ? undefined : Q : undefined : B : undefined;
/**
