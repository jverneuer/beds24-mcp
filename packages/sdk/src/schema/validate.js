...
function collectFieldNames(node, out) {
    ...
    if (Array.isArray(n.allOf)) {
        for (const m of n.allOf)
            collectFieldNames(m, out);
    }
    if (Array.isArray(n.oneOf)) {
        for (const m of n.oneOf)
            collectFieldNames(m, out);
    }
...
function suggestField(given, known) {
    if (known.size === 0)
        return undefined;
    let best;
...
function toValidationError(err, knownFields) {
    ...
    if (keyword === "type" || keyword === "format") {
        const expected = err.params.type ?? keyword;
        return {
            path,
...
export class Beds24Validator {
    ...
    static create(opts) {
        return new Beds24Validator(opts?.specDir ?? defaultSpecDir());
    }
    /**
     ...
     */
    validate(endpoint, direction, payload) {
        const schema = getSchema(endpoint, direction, this.specDir);
        ...
        try {
            ...
            return {
                ...
                errors: [
                    {
                        path: "(root)",
                        message: `failed to compile schema for "${endpoint}": ${e.message}`,
                    },
                ],
            };
        }
 ...
 * or `{ valid, errors }`.
 */
export function validateRequest(endpoint, direction, payload, specDir = defaultSpecDir()) {
    return Beds24Validator.create({ specDir }).validate(endpoint, direction, payload);
}
