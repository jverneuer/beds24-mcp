...
const indexCache = new Map<string, SchemaIndex>();
 ...
 */
function getIndex(specDir: string = defaultSpecDir()): SchemaIndex {
...
export function getSchema(
	...
	specDir: string = defaultSpecDir(),
): JsonNode | undefined {
	return getIndex(specDir).getSchema(endpoint, direction);
}

/** Resolve a named component schema into a normalized object. */
export function resolveNamedSchema(name: string, specDir: string = defaultSpecDir()): JsonNode | undefined {
	return getIndex(specDir).resolveNamed(name);
}

/** List every `METHOD /path` in the spec. */
export function listEndpoints(specDir: string = defaultSpecDir()): string[] {
 ...
 */
export function resolveSchema(name: string, specDir: string = defaultSpecDir()): Field[] {
