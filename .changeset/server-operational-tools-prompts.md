...
Add the OPERATE surface to the MCP server: per-request `Beds24Client`-backed operational tools, workflow prompts, server instructions, and full tool/prompt/resource test coverage.

Operational tools (each takes an `auth` block of `refreshToken` | `inviteCode` | `token`, builds a per-request `Beds24Client`, and delegates to the matching SDK ops): `beds24_booking_get/create/cancel`, `beds24_booking_message_list/send`, `beds24_price_set_daily/get_calendar/set_fixed`, `beds24_availability_get`, `beds24_inventory_offers`, `beds24_property_list`, `beds24_account_list`, `beds24_channel_settings_get/configure`, `beds24_webhook_register`.
